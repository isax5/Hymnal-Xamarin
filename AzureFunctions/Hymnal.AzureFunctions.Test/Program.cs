...
{
    //private static Subject<int> sInt = new();
    //private static IObservable<int> observableInt => sInt.AsObservable();
    ...
    static async Task Main(string[] args)
    {
        ...
        */
        ...
        var subs1 = observable
            .Subscribe(
            x => Console.WriteLine($"New Value: {x}"),
        ...
        while (true)
        {
            service.ObserveSettings()
                .SelectMany(x => x)
                .Subscribe(x => Console.WriteLine($"New value loop: {x}"), ex => Console.WriteLine($"Problema en subscripción ciclica: {ex}"));
